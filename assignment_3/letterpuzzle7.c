...
void generatePermutations(char str[], int start, int end, char permutations[][10], int *count) {
    if (start == end) {
...
int main() {
    ...
    char permutations[factorial][10];
    int count = 0;

    generatePermutations(str, 0, n - 1, permutations, &count);

    qsort(permutations, count, sizeof(char[10]), compareStrings);
