...
{
    LinkedList original = list;                                            // Create a copy of the original list
    AccountInfo *accountInfo = (AccountInfo *)malloc(sizeof(AccountInfo)); // Create a new account info structure
    LinkedList newNode = (LinkedList)malloc(sizeof(AccountNode));          // Create a new node for the account
    ...
    while (check != NULL)
    {
        ...
        if (check->data.type == accountType && strcmp(check->data.name, name) == 0)
        {
            ...
            return original; // Return the original list
        }
        check = check->next; // Traverse the list
    }
...
void transaction(int accountNumber, float amount, int code, LinkedList list)
{
    ...
    int found = 0;
    // Traverse the list to find the account to be updated
    while (list != NULL)
    {
        // If the account is found
